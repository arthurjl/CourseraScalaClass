...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw java.util.NoSuchElementException("Empty Set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    val new_acc = if p(elem) then acc.incl(elem) else acc 
  ...
  def mostRetweeted: Tweet =
    var most = elem
    foreach((x) => if x.retweets > most.retweets then most = x)
    most
